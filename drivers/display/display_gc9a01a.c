 ...
 * @return 0 on success, errno otherwise.
 */
int gc9a01a_transmit(const struct device *dev, uint8_t cmd, const void *tx_data, size_t tx_len)
{
	const struct gc9a01a_config *config = dev->config;
 ...
 *
 * @param dev GC9A01A device instance.
 * @return 0 when successful, errno otherwise.
 */
 ...
 * @return 0 when successful, errno otherwise.
 */
static int gc9a01a_set_mem_area(const struct device *dev, const uint16_t x, const uint16_t y,
 ...
 * @return 0 when successful, errno otherwise.
 */
static int gc9a01a_write(const struct device *dev, const uint16_t x, const uint16_t y,
 ...
 */
static int gc9a01a_read(const struct device *dev, const uint16_t x, const uint16_t y,
...
#define GC9A01A_INIT(n, t)                                                                         \
	...
	static const struct gc9a01a_config gc9a01a_config_##n = {                                  \
		.spi = SPI_DT_SPEC_GET(INST_DT_GC9A01A(n), SPI_OP_MODE_MASTER | SPI_WORD_SET(8),   \
				       0),                                                         \
		.cmd_data = GPIO_DT_SPEC_GET(INST_DT_GC9A01A(n), cmd_data_gpios),                  \
		.reset = GPIO_DT_SPEC_GET_OR(INST_DT_GC9A01A(n), reset_gpios, {0}),                \
	...
	DEVICE_DT_DEFINE(INST_DT_GC9A01A(n), gc9a01a_init, NULL, &gc9a01a_data_##n,                \
			 &gc9a01a_config_##n, POST_KERNEL, CONFIG_DISPLAY_INIT_PRIORITY,           \
			 &gc9a01a_api)
